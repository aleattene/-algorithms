...
export function getDijkstraMinPath(
	distancesGraph: Graph,
	start: string,
	end: string,
) {
	...
	while (nodes.length) {
		...
		for (const neighbor in distancesGraph[currentNode]) {
			const newDistance =
				currentDistance + distancesGraph[currentNode][neighbor];
	...
	for (let i = 0; i < minPath.path.length - 1; i++) {
		minPath.distance +=
			distancesGraph[minPath.path[i]][minPath.path[i + 1]];
	}
